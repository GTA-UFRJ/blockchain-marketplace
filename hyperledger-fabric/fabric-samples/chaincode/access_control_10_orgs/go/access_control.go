...
func (t *SimpleChaincode) Init(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if len(args) != 20 {
		return shim.Error("Incorrect number of arguments. Expected 20 arguments")
	}
	/*if len(args[0]) <= 0 {
	...
	*/
    ...
    for i := 0; i<10; i++ {
        ...
        contractJSONasString := `{"org": "` + orgs[i] +`","assets": "` + assets[i] +`"}`
        contractJSONasBytes:= []byte(contractJSONasString)

        // Save transaction to global state
        err = stub.PutState(orgs[i], contractJSONasBytes)
        if err != nil {
